...
//        var_dump($model->lft);
       $id =GoodsCategory::find()->andWhere(['=','tree',$model->tree])->andWhere(['>','lft',$model->lft])->andWhere(['<','rgt',$model->rgt])->all();
         $cate=ArrayHelper::map($id,'id','id');
        $mode=Goods::find()->where(['goods_category_id'=>$cate])->all();
//        var_dump($mode);
//        exit;



        return $this->render('list',['model'=>$mode]);
    }
    public function actionXiang($id){
        $model=Goods::findOne(['id'=>$id]);

        return $this->render('xiang',['model'=>$model]);
    }
    //购物车
...
//            $cart = [$goods_id=>$amount];
    ...
    public function actionShop()
    {
        ...
        $order->trane_no = $b = mt_rand(100000, 999999);
        $order->status = 1;

...
//    }
    ...
    public function actionMemberLogin(){
        $request=\Yii::$app->request;
        if($request->isPost){
            $member=new LoginFrom();
            $member->scenario=LoginFrom::SCENARIO_API_LOGIN;
            $member->username=$request->post('username');
            $member->password=$request->post('password');
            $member->code=$request->post('code');
            $member->remenber=0;
//            $member->code=$request->post('cod
...
//            var_dump($brand_id);die;
    ...
    public function actionArticleByCate(){
        ...
        if($request->isGet){
            ...
            $article=Article::find()->where(['id'=>$cate_id])->asArray()->all();
            return ['status'=>1,'msg'=>'查询成功','data'=>$article];
    ...
    public function actionCateByArticle(){
        ...
        if($request->isGet){
            ...
            $cate=Article_category::findOne(['id'=>$article->id]);
        ...
        if(!\Yii::$app->user->isGuest){
            if(\Yii::$app->request->isPost){
                ...
                $order->tel=$address->tel;
                    ...
                    $order->delivery_name=Order::$deliveries[$delivery_id-1]['kd'];
                    $order->delivery_price=Order::$deliveries[$delivery_id-1]['jq'];
                    $payment_id=\Yii::$app->request->post('payment_id');
                    $order->payment_id=Order::$payments[$payment_id-1]['id'];
                    $order->payment_name=Order::$payments[$payment_id-1]['kd'];
